...
import { mockSkills, mockComments, FileNode, CURRENT_USER_ID } from '../data/mockData';
...
export function SkillDetail() {
  ...
  const [selectedVersionForDownload, setSelectedVersionForDownload] = useState<string | null>(null);

  // Check if current user is the author
  const isAuthor = skill?.author.id === CURRENT_USER_ID;
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200/50">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="font-semibold text-lg">版本历史</h2>
                </div>
                <span className="text-sm text-slate-500">
                  共 {skill.versionHistory.length} 个版本
                </span>
              </div>
              <div className="space-y-3">
                {skill.versionHistory.map((version, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-4 text-xs text-slate-500">
                      <span>发布于 {version.releaseDate}</span>
                      <span>·</span>
